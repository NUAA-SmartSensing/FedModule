...
class CheckInThread(threading.Thread):
    def __init__(self, checkin_config, sync_client_manager, current_t, t):
    ...
    def run(self):
        ...
        while self.current_t.get_time() < self.T:
            ...
            # 每隔一段时间就有新的client checker in
            if self.sync_client_manager.get_unchecked_in_client_thread_list_len() > 0 and self.check_in_num > 0:
                ...
                if current_time >= (self.check_in_interval * c_count) and current_time != last_c_time:
                    ...
                    waiting_c_n += self.sync_client_manager.client_check_in(self.check_in_num)
