...
class BaseClientManager:
    def __init__(self, init_weights, clients_num, multi_gpu, datasets, current_time, schedule_t, stop_event,
    ...
    def init_clients(self):
        client_class = ModuleFindTool.find_class_by_path(self.config["client_path"])
        # 初始化clients
        # 0: 多gpu，1：单gpu，2：cpu
        ...
        for i in range(self.clients_num):
            ...
            self.client_thread_list.append(
                client_class(i, self.queue_manager, self.stop_event, client_delay, dataset, self.client_config, dev,
                             self.print_lock,
                             self.global_var))
