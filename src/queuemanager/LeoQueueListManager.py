...
class LeoQueueListManager(BaseQueueManager):
    ...
    def receive(self, nums, *args, **kwargs):
        # nums 即一个装载该轮次多少个
        # group_ready_num是上一轮训练好，待聚合的group_id，也是FedAT下开启下一轮的group_id
        self.group_id_next = self.global_var['access_group_info']['Orbit'].iloc[self.global_var['current_t'].get_time() - 1]
        self.group_ready_list = self.receiver_caller.receive(self.queue, nums, self.group_id_next, *args, **kwargs)
        self.group_ready_num = self.group_id_next
